...
SQL_BLOCKS = [
    ...
    create or replace view job_scrape.working_student_jobs_v as
    select *
    from job_scrape.target_jobs_m
    where is_working_student
      and is_active
      and target_role_confidence in ('high', 'medium');

    -- Map-only location expansion layer for the conservative working-student dataset.
    create or replace view job_scrape.working_student_location_candidates_v as
    with base as (
      select
        d.platform,
        d.job_id,
      ...
      from job_scrape.working_student_jobs_v d
    ...
    create or replace view job_scrape.working_student_map_points_v as
    with c as (
      select * from job_scrape.working_student_location_candidates_v
    ),
    joined as (
      select
        d.*,
      ...
      from c
      join job_scrape.working_student_jobs_v d
    ...
    create or replace view job_scrape.working_student_city_bubbles_v as
    with pts as (
      select *
      from job_scrape.working_student_map_points_v
    ...
    create or replace view job_scrape.working_student_kpis_v as
    with base as (
      select
        platform,
        job_id,
        ...
        first_seen_at,
        last_seen_at
      from job_scrape.working_student_jobs_v
    ),
    remote_jobs as (
      select distinct c.platform, c.job_id
      from job_scrape.working_student_location_candidates_v c
      join base b
        on b.platform = c.platform and b.job_id = c.job_id
    ...
    create or replace view job_scrape.working_student_top_skills_v as
    with base as (
      select
        platform,
        job_id,
    ...
    create materialized view if not exists job_scrape.working_student_map_points_m as
    select *
    from job_scrape.working_student_map_points_v
    with no data;

    refresh materialized view job_scrape.working_student_map_points_m;

    create unique index if not exists idx_wsmp_job_point
      on job_scrape.working_student_map_points_m (map_point_id);
    create index if not exists idx_wsmp_kind_lat_lon
      on job_scrape.working_student_map_points_m (token_kind, lat, lon);
    create index if not exists idx_wsmp_job
      on job_scrape.working_student_map_points_m (platform, job_id);
    create index if not exists idx_wsmp_posted_at
      on job_scrape.working_student_map_points_m (posted_at_utc);
    create index if not exists idx_wsmp_first_seen
      on job_scrape.working_student_map_points_m (first_seen_at);
    """,
    r"""
    create schema if not exists job_scrape;
    set statement_timeout = '30min';

    create materialized view if not exists job_scrape.working_student_city_bubbles_m as
    with pts as (
      select *
      from job_scrape.working_student_map_points_m
      where token_kind = 'city'
        and lat is not null and lon is not null
    ),
    base as (
    ...
    refresh materialized view job_scrape.working_student_city_bubbles_m;

    create index if not exists idx_wscb_lat_lon
      on job_scrape.working_student_city_bubbles_m (lat, lon);
    create index if not exists idx_wscb_new7d
      on job_scrape.working_student_city_bubbles_m (new_7d_unique desc);
    create index if not exists idx_wscb_new24h
      on job_scrape.working_student_city_bubbles_m (new_24h_unique desc);
    """,
    r"""
    create schema if not exists job_scrape;
    set statement_timeout = '30min';

    create materialized view if not exists job_scrape.working_student_trend_m as
    select *
    from job_scrape.working_student_trend_v
    with no data;
    refresh materialized view job_scrape.working_student_trend_m;

    create materialized view if not exists job_scrape.working_student_top_skills_m as
    select *
    from job_scrape.working_student_top_skills_v
    with no data;
    refresh materialized view job_scrape.working_student_top_skills_m;

    create index if not exists idx_wsts_days_jobs
      on job_scrape.working_student_top_skills_m (days_window, jobs_unique desc);
    """,
]
    ...
    with connect() as conn:
        with conn.cursor() as cur:
            for sql in SQL_BLOCKS:
                cur.execute(sql)
                conn.commit()
    print("working_student_app_views_ready")
