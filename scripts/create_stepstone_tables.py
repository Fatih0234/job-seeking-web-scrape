...
create table if not exists job_scrape.stepstone_crawl_runs (
...
create table if not exists job_scrape.stepstone_search_definitions (
  id uuid primary key default gen_random_uuid(),
...
create table if not exists job_scrape.stepstone_search_runs (
  id uuid primary key default gen_random_uuid(),
  crawl_run_id uuid not null references job_scrape.stepstone_crawl_runs(id),
  search_definition_id uuid not null references job_scrape.stepstone_search_definitions(id),
  status text not null default 'running',
...
create table if not exists job_scrape.stepstone_jobs (
...
create table if not exists job_scrape.stepstone_job_search_hits (
  search_run_id uuid not null references job_scrape.stepstone_search_runs(id),
  job_id text not null references job_scrape.stepstone_jobs(job_id),
...
create table if not exists job_scrape.stepstone_job_details (
  job_id text primary key references job_scrape.stepstone_jobs(job_id),
  scraped_at timestamptz not null,
  job_title text,
  company_name text,
  job_location text,
