...
def create_crawl_run(trigger: str) -> str:
    with connect() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "insert into job_scrape.xing_crawl_runs (trigger, status) values (%s, 'running') returning id",
                (trigger,),
            )
            (run_id,) = cur.fetchone()
        conn.commit()
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                select id, name, keywords, country_name, location_text, facets
                  from job_scrape.xing_search_definitions
                 where enabled = true
                 order by name asc
                """
            )
    ...
    with connect() as conn:
        with conn.cursor() as cur:
            for s in searches:
                cur.execute(
                    """
                    insert into job_scrape.xing_search_runs (crawl_run_id, search_definition_id, status)
                    values (%s, %s, 'running')
                ...
                if row:
                    ...
                    cur.execute(
                        "select id from job_scrape.xing_search_runs where crawl_run_id=%s and search_definition_id=%s",
                        (crawl_run_id, s["search_definition_id"]),
                    )
                    (rid,) = cur.fetchone()
